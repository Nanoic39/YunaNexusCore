...
public interface UserFileShardMapper {
        ...
        @Select("""
                        ...
                        LIMIT #{limit}
                        """)
        List<YunaFile> listByUser(@Param("table") String table, @Param("userId") Long userId,
        ...
        @Select("""
                        ...
                        """)
        List<YunaFile> selectByFolder(@Param("table") String table, @Param("userId") Long userId,
        ...
        @Update("""
                        ...
                        WHERE uuid = #{uuid} AND user_id = #{userId}
                        """)
        int updateFolderId(@Param("table") String table, @Param("uuid") String uuid, @Param("userId") Long userId,
                        @Param("folderId") Long folderId);
        ...
        @Select("""
                        ...
                        LIMIT #{limit}
                        """)
        List<YunaFile> selectRecycleBin(@Param("table") String table, @Param("userId") Long userId,
                        @Param("limit") int limit);

        @Select("""
                        SELECT * FROM ${table}
                        WHERE status = 1 AND delete_time < DATE_SUB(NOW(), INTERVAL 30 DAY)
                        LIMIT 100
                        """)
        List<YunaFile> selectExpiredForHide(@Param("table") String table);

        @Select("""
                        SELECT * FROM ${table}
                        WHERE status = 2 AND delete_time < DATE_SUB(NOW(), INTERVAL 180 DAY)
                        LIMIT 100
                        """)
        List<YunaFile> selectExpiredForDelete(@Param("table") String table);
}